...
import { getMappedPurpose } from "./getMappedPurpose.js";
import { resetEmptyString } from "./string.js";

interface GetMappedAuthProps {
  config: ConfigObject;
  device: Device;
}
export function getMappedAuth({
  config,
  device,
}: Readonly<GetMappedAuthProps>): any {
  const mappedAuth = {
    ...
    reference: {
      ...
      transaction: resetEmptyString(
        config.transaction?.reference ?? config.order?.reference
      ),
    },
