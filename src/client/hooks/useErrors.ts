...
export const useErrors = () => {
  ...
  const {
    ...
    errorText: cvvErrorText,
  } = useSelector(getCVV);

  const { isValid: isHolderValid, isUserDoneTyping: isHolderCompleteTyping } =
    useSelector(getHolderName);
  const cardFundingSource =
    config.paymentOptions?.cardFundingSource !== undefined
      ? config.paymentOptions?.cardFundingSource
      : "all";

  const isFundingSourceValid =
    BIN === null ||
    (BIN !== null && cardFundingSource === "all") ||
    BIN.card_type === cardFundingSource.toUpperCase();
  ...
  const isFundingSourceValidErrorMessage = useMemo(() => {
    if (!isFundingSourceValid && BIN !== null) {
      ...
      const errorTranslated = t("Hints.Error.wrongFundSource").replace(
        "%@",
        t(fsTranslationKey),
      );
  ...
  const currentCardType = cardType?.type
    .toUpperCase()
    .replace("-", "_") as unknown as CardBrands;
  const currentSupportedCards = clickToPay?.enabled
    ? clickToPaySupportedCards
    : supportedCards;
  const isInCardSupported = currentSupportedCards.find(
    (card) =>
      card.name === currentCardType ||
      (BIN !== null && card.name === BIN.card_scheme),
  )
    ? true
    : false;
  ...
  const isCardSupported =
    isInCardSupported ||
    (config.integration === Integration.CHECKOUT &&
      !clickToPay?.enabled &&
      isInCardDisabled);
  const isCreditCardError =
    (isCreditCardCompleteTyping &&
      !isCreditCardValid &&
      !isCreditCardPotentiallyValid) ||
    (isCreditCardCompleteTyping && !isCardSupported);

  const isExpireDateError =
    isExpireDateCompleteTyping && !isExpireDateValid && !isCreditCardError;
