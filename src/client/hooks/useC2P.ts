	...
	const encrypt = useCallback(async ({ cardNumber, cvv, date }: CardData) => {
		...
		try {
			if (!MastercardCheckoutServices) {
				...
				throw { reason: 'MastercardCheckoutServices is not defined', name: 'MastercardCheckoutServices' }
			}
			...
			const { encryptedCard, cardBrand } = await mcInstance.encryptCard(cardData)
