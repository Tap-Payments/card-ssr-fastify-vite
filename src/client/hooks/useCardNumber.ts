...
import type { CreditCardTypeSecurityCodeLabel } from '../types/cardtypeI'
import { getConfig } from '../features/configSlice'
import { getCard, setBIN, setBIN_LOCKED, setCardValue, setIsInEnglish, setType } from '../features/cardSlice'
import { allCharsInEN, getCardType, maskStringFromCardNumber, removeWhitespaces, sendEventGeneric } from '../utils'
...
import { CardFundingSource, configPaymentOption } from '../types'
...
export const useCardNumber = () => {
	...
	const placeholder = useMemo(() => {
		...
		return t('TapCardInputKit.enterCardLabel').replace('%@', t(fsTranslationKey))
	}, [t, cardFundingSource])

	const onChangeCardNumber = async (value: string) => {
		// max allowed card number length is 16 "only take first 16"
		const valueTrimmed = removeWhitespaces(value).substring(0, CREDIT_CARD_NUMBER.maxAllowedLength)
		...
		if (valueTrimmed.length > 9 && !valueTrimmed.startsWith('0')) {
			if (BIN_LOCKED === false) {
				...
				try {
					...
					if (!isValidCardBrand || !isValidSchema) return dispatch(setCardValue(''))
