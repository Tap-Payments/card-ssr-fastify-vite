...
export const useTheme = () => {
  ...
  const getFontFormat = (themeFormat: string, forceEN = false): string => {
    ...
    if (formatArray.length === 2)
      return `${formatArray[1]}px ${formatArray[0]}`;
  ...
  const getColorProperty = (
    color: string | keyof typeof theme.GlobalValues.Colors,
  ): string => {
    return (
      theme.GlobalValues.Colors[
        color as keyof typeof theme.GlobalValues.Colors
      ] || color
    );
  };
