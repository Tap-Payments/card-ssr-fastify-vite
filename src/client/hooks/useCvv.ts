...
export const useCvv = () => {
  ...
  const onChangeCvv = async (value: string) => {
    ...
    if (value.length === size) {
      ...
      if (!loadedCard) {
        dispatch(
          setPrevCvvState({ value, isValid: true, isUserDoneTyping: true }),
        );
      }
    }
    ...
    if (value.length < size) {
      ...
      if (!loadedCard) {
        dispatch(
          setPrevCvvState({ value, isValid: false, isUserDoneTyping: false }),
        );
      }
    }
    ...
    if (value.length === 0) {
      dispatch(
        setCvvErrorText(
          t("Hints.Warning.missingCVV").replace("%i", size.toString()),
        ),
      );
      dispatch(setCvvIsValid(false));
      dispatch(setIsUserDoneTyping(true));
