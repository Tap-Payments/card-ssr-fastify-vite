    ...
    useEffect(() => {
        import('@tap-payments/card-web')
            .then((module) => {
                ...
                (window as any).generateTapToken = () => { module.tokenize(); };
                (window as any).loadAuthentication = (authUrl: string) => { module.loadAuthentication(authUrl); };
                (window as any).cancelAuthentication = () => { module.cancelAuthentication(); };
                (window as any).fillCardInputs = (cardInputs: any) => { module.fillCardInputs(cardInputs); };
                (window as any).setIP = (ip: string) => { module.sendIP(ip); };
            })
            .catch((error) => {
