...
const overWriteCardsLength = () => {
  ...
  for (const cardBrandId of cardBrandIds) {
    ...
    const allowedLengths = info?.lengths?.filter(
      (length) => length <= CREDIT_CARD_NUMBER.maxAllowedLength,
    );
...
export const getCardType = (value: string): ReturnCardType => {
  ...
  const trimmedValue = removeWhitespaces(value).substring(
    0,
    CREDIT_CARD_NUMBER.maxAllowedLength,
  );
  if (trimmedValue === "")
    return { type: null, isValid: false, isPotentiallyValid: false };
 ...
 */
export function maskStringFromCardNumber({
  gaps,
  lengths,
}: MaskFunction): string {
  // 0. if gaps or lengths are empty return default mask.
  if (gaps.length === 0 || lengths.length === 0)
    return CREDIT_CARD_NUMBER.defaultMask;
