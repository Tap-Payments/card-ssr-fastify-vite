...
export const isFuture = (value: string) => {
  const { isValid, isPotentiallyValid, month, year } = valid.expirationDate(
    value,
    100,
  );
  return {
...
export const validateAppConfig = (values: AppProps) => {
  ...
  if (
    !paymentOptions.find((payment) =>
      payment.supported_currencies.includes(
        String(configValue?.paymentOptions?.currencyCode) as CURRENCIES,
      ),
    )
  ) {
    return {
  ...
  const authData: Auth = JSON.parse(
    decodeURIComponent(configValue.authentication as unknown as string),
  );
  const purpose = authData?.authentication.purpose;
  const merchantID = authData?.merchant?.id;
  const disableValidateMerchant =
    integrationMode === Integration.WEBVIEW &&
    (!configValue.sdkVersion || Number(configValue.sdkVersion) < 1);
