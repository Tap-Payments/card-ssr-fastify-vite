...
const createAuthentication = async ({
	...
	const obj = {
		...
		customer: tokenResponse.customer ?? mapCustomerToApi(authentication?.customer),
		payment_agreement: tokenResponse.payment_agreement ?? authentication?.paymentAgreement,
		device: {
