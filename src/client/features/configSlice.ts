...
import { Scope, ThemeMode } from '../types'
import type { DarkTheme } from '../types/dark'
import type { LightTheme } from '../types/light'
import type { Authentication } from '../types/Authentication'
import type { LocalizationObject } from '../types/localization'
import type { Features, configPaymentOption, configProps } from '../types/configProps'
import type { CURRENCIES, PaymentOption, SupportedCards } from '../types/paymentOption'
import API from '../api'
import I18n from '../i18n'
import { getAcceptedCards, getClickToPaySupportedCards, getNotAcceptedCards } from '../utils/paymentMethod'
...
export const fetchLocaleAsync = createAsyncThunk('localization/get', async (url: string) => {
...
export const configSlice = createSlice({
	...
	initialState,
	reducers: {
		setHideSavedCardForLoading: (state, action: PayloadAction<ConfigSliceProps['hideSavedCardForLoading']>) => {
			state.hideSavedCardForLoading = action.payload
		},
		setHideErrorFooter: (state, action: PayloadAction<ConfigSliceProps['hideErrorFooter']>) => {
		...
		setSaveCardOption: (state, action: PayloadAction<configPaymentOption['saveCardOption']>) => {
		...
		setReferer: (state, action: PayloadAction<ConfigSliceProps['refererUrl']>) => {
			state.refererUrl = action.payload
		},
		setEncryptionKey: (state, action: PayloadAction<ConfigSliceProps['encryptionKey']>) => {
			state.encryptionKey = action.payload
		},
		setConfigPaymentOption: (state, action: PayloadAction<configPaymentOption>) => {
			state.config.paymentOptions = { ...state.config.paymentOptions, ...action.payload }
			...
			if (currencyCode && state.authentication) {
				const currency = Array.isArray(currencyCode) ? currencyCode[0] : currencyCode
			...
			if (
				isSupportedCurrency({
					currencyCode,
					supportedCurrencies: [action.payload?.sortedCurrencyCode as CURRENCIES]
				})
			) {
				state.allCards = [
		...
		setPaymentOptions: (state, action: PayloadAction<Array<PaymentOption>>) => {
			...
			state.clickToPaySupportedCards = clickToPaySupportedCards.map((card) => ({ ...card, isDisabled: false }))
			// filter out payment_type==="card"
			const paymentOptions = action.payload.filter((item) => item.payment_type !== 'card')
		...
		setLanguages: (state, action: PayloadAction<ConfigSliceProps['languages']>) => {
			state.languages = action.payload
		},
		updateLanguage: (_, action) => {
			I18n.changeLanguage(action.payload)
		},
		setAuthentication: (state, action: PayloadAction<ConfigSliceProps['authentication']>) => {
			state.authentication = action.payload
		},
		setScope: (state, action: PayloadAction<ConfigSliceProps['scope']>) => {
			state.scope = action.payload
		},
		setFeatures: (state, action: PayloadAction<ConfigSliceProps['features']>) => {
		...
		setSavedCardCVV: (state, action: PayloadAction<ConfigSliceProps['savedCardCVV']>) => {
			state.savedCardCVV = action.payload
		},
		setClickToPay: (state, action: PayloadAction<ConfigSliceProps['clickToPay']>) => {
